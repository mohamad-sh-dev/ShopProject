        ...
        switch (_context.prev = _context.next) {
          case 0:
            ...
            return (0, _axios.default)({
              method: "patch",
              url: url,
              data: data // send data who can be password or data(name,email)
...
if (mapBox) {
  ...
  (0, _script.displayMap)(locations);
} // if element does exists


if (loginForm) {
  loginForm.addEventListener("submit", function (el) {
    el.preventDefault(); //get values of our website inputes and set to variables(name,password)

    var email = document.getElementById('email').value;
    var password = document.getElementById('password').value; //send to our backend for login (API)
...
if (logOutBtn) logOutBtn.addEventListener("click", _login.logout); // update user data(email,name) and set type to "data"

if (updateUser) {
  updateUser.addEventListener("submit", function (el) {
    el.preventDefault(); //get values of our website inputes and set to variables(name,email)

    var form = new FormData();
    form.append("name", document.getElementById("name").value);
    form.append("email", document.getElementById("email").value);
    form.append("photo", document.getElementById("photo").files[0]); // send values to our backend(API)

    (0, _updateUserData.updateUserData)(form, "data");
  });
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              ...
              passwordConfirm = document.getElementById("password-confirm").value; // send values to our backend (API)
